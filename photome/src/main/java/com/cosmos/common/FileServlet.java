...
public class FileServlet extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException {
		...
		this.basePath = StringUtils.substringBeforeLast(request.getPathInfo(), "/");

		if (this.basePath == null) {
			throw new IOException("FileServlet init param 'basePath' is required.");
		} else {
			File path = new File(this.basePath);
			if (!path.exists()) {
				throw new IOException("FileServlet init param 'basePath' value '" + this.basePath + "' does actually not exist in file system.");
			} else if (!path.isDirectory()) {
				throw new IOException("FileServlet init param 'basePath' value '" + this.basePath + "' is actually not a directory in file system.");
			} else if (!path.canRead()) {
				throw new IOException("FileServlet init param 'basePath' value '" + this.basePath + "' is actually not readable in file system.");
			}
		}

		// Get requested file by path info.
		// String requestedFile = request.getPathInfo();
		String requestedFile = StringUtils.substringAfterLast(request.getPathInfo(), "/");
